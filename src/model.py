...
# test function generated by Gemini
def test_transformer():
    ...
    D_EMBED = 512
    D_HIDDEN = 512
    D_FF = 2048 # Typically D_EMBED * 4
    N_HEAD = 8
    DROPOUT = 0.1

    # Instantiate the model
    # Note: Ensure the DecoderOnlyTransformer class is available in the current scope
    ...
    # 1. Test Standard Input (bs=2, seq_len=10)
    BS = 2
    SEQ_LEN = 10
    dummy_input = torch.randint(0, VOCAB_SIZE, (BS, SEQ_LEN))
    ...
    try:
        ...
        # Validate shape: (Batch Size, Sequence Length, Vocab Size)
        expected_shape = (BS, SEQ_LEN, VOCAB_SIZE)
        assert output_logits.shape == expected_shape, \
            f"Shape Error! Expected: {expected_shape}, Actual: {output_logits.shape}"

        print(f"✅ Standard Input Test Passed (bs={BS}, seq_len={SEQ_LEN})")

    except Exception as e:
        print(f"❌ Standard Input Test Failed: {e}")
        return

    # 2. Test Maximum Sequence Length (seq_len=MAX_SEQ_LEN)
    SEQ_LEN_MAX = MAX_SEQ_LEN
    dummy_input_max = torch.randint(0, VOCAB_SIZE, (BS, SEQ_LEN_MAX))

    try:
        with torch.no_grad():
            output_logits_max = model(dummy_input_max)

        expected_shape_max = (BS, SEQ_LEN_MAX, VOCAB_SIZE)
        assert output_logits_max.shape == expected_shape_max, \
            f"Max Length Shape Error! Expected: {expected_shape_max}, Actual: {output_logits_max.shape}"

        print(f"✅ Maximum Length Test Passed (seq_len={SEQ_LEN_MAX})")

    except Exception as e:
        print(f"❌ Maximum Length Test Failed: {e}")
        return

    print("--- All tests completed successfully! Model structure and tensor shapes are correct. ---")
